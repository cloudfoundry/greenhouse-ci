...
govc vm.clone -verbose -vm ${BASE_VM_IPATH} -ds ${CLONE_DATASTORE} -pool ${CLONE_RESOURCE_POOL} -folder "$CLONE_FOLDER" -on=false "$CLONE_NAME"
govc vm.customize -verbose -vm.ipath "$CLONE_FOLDER"/"$CLONE_NAME" -ip ${VM_IP} ${VM_CUSTOMIZATION_NAME}
govc vm.power -verbose -on -vm.ipath "$CLONE_FOLDER"/"$CLONE_NAME"
